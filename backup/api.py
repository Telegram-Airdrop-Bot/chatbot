...
def add_user(user_id, full_name, username, join_date, invite_link, photo_url=None):
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
...
def dashboard_users():
    ...
    c.execute('SELECT COUNT(*) FROM users')
    total = c.fetchone()[0]
    c.execute('SELECT user_id, full_name, username, join_date, invite_link, photo_url FROM users ORDER BY join_date DESC LIMIT ? OFFSET ?', (page_size, offset))
    users = c.fetchall()
    conn.close()
        ...
        try:
            ...
            if images:
                if len(images) > 1:
                    ...
                    fut.result()
                    save_message(user_id, 'admin', f'[Images sent: {len(images)} files]')
                else:
                    ...
                    fut.result()
                    save_message(user_id, 'admin', f'[Image sent: {files[0].filename}]')
                sent = True
            if videos:
                if len(videos) > 1:
                    ...
                    fut.result()
                    save_message(user_id, 'admin', f'[Videos sent: {len(videos)} files]')
                else:
                    ...
                    fut.result()
                    save_message(user_id, 'admin', f'[Video sent: {files[0].filename}]')
                sent = True
            if audios:
                if len(audios) > 1:
                    ...
                    fut.result()
                    save_message(user_id, 'admin', f'[Audios sent: {len(audios)} files]')
                else:
                    ...
                    fut.result()
                    save_message(user_id, 'admin', f'[Audio sent: {files[0].filename}]')
                sent = True
        except Exception as e:
